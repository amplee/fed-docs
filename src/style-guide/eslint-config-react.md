...
- 基于webpack、babel自建的react项目
- 基于create-react-app搭建的react项目
- 基于umiJS 搭建的react项目

并添加了对 `typescript`的支持。
...
``` js
module.exports = {
    extends: '@bestwehotel/eslint-config-react'
};
```
...
``` sh
npm i -D eslint babel-eslint eslint-config-standard eslint-plugin-import eslint-plugin-node eslint-plugin-promise eslint-plugin-standard @typescript-eslint/eslint-plugin @typescript-eslint/parser confusing-browser-globals eslint-plugin-jsx-a11y eslint-plugin-react eslint-plugin-react-hooks
```
